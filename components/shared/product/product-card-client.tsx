...
const ProductCardClient = ({ product, isSignedIn, initialWishlisted }: ProductCardClientProps) => {
  ...
  const rotateX = useSpring(useTransform(rawY, [-0.5, 0.5], [7, -7]), { stiffness: 300, damping: 28 });
  const rotateY = useSpring(useTransform(rawX, [-0.5, 0.5], [-7, 7]), { stiffness: 300, damping: 28 });
  ...
  const numCompareAt = product.compareAtPrice ? Number(product.compareAtPrice) : null;
  ...
  return (
    <motion.div
      ...
      className="group relative w-full max-w-sm overflow-hidden rounded-2xl border bg-card text-card-foreground shadow-sm transition-shadow duration-300 hover:shadow-xl"
    >
      <div className="relative aspect-4/5 w-full overflow-hidden bg-muted">
        ...
        <Link href={`/product/${product.slug}`} className="absolute inset-0 block" tabIndex={-1}>
          <motion.div
            variants={{ rest: { scale: 1 }, hover: { scale: reducedMotion ? 1 : 1.06 } }}
          ...
          {secondImage && (
            <motion.div
              variants={{ rest: { opacity: 0 }, hover: { opacity: reducedMotion ? 0 : 1 } }}
          ...
          {colors.length > 0 && (
            <div className="mt-2 flex flex-wrap items-center gap-1">
              {colors.slice(0, 4).map((color) => (
                <Badge key={color} variant="outline" className="bg-background/80 text-[10px] font-normal">
                  {color}
                </Badge>
              ))}
              {colors.length > 4 && (
      ...
      <div className="grid gap-1.5 p-4">
        ...
        <Link href={`/product/${product.slug}`}>
          <h2 className="line-clamp-1 text-sm font-medium transition-colors group-hover:text-primary">{product.name}</h2>
        </Link>
        ...
        <div className="mt-1 flex items-baseline gap-2">
          {hasCompareAt && (
            <span className="text-xs text-muted-foreground line-through">${numCompareAt!.toFixed(2)}</span>
          )}
          {product.stock > 0 ? (
            <ProductPrice value={numPrice} className={cn(hasCompareAt && 'text-destructive')} />
          ) : (
