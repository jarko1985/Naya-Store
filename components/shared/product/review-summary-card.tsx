...
const ReviewSummaryCard = ({ rating, numReviews, reviews }: ReviewSummaryCardProps) => {
  if (numReviews === 0) {
    return (
          ...
          <div className='space-y-3 pt-3 border-t border-border'>
            {topReviews.map((review) => (
              <div key={review.id}>
                ...
                <p className='text-sm font-medium mt-1 line-clamp-1'>{review.title}</p>
                <p className='text-xs text-muted-foreground line-clamp-2 mt-0.5'>{review.description}</p>
                {review.user?.name && (
                  <p className='text-[11px] text-muted-foreground mt-1'>— {review.user.name}</p>
                )}
              </div>
            ))}
          </div>
